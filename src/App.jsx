...
/* ===== 定数 ===== */
const customLabel = '60以下';                              // 60 以下ボタン
...
export default function App() {
  ...
  const [dims, setDims]   = useState({ l:'', w:'', h:'' });   // l=縦, w=横, h=高さ
  ...
  useEffect(() => {
    ...
    setSize(auto);
    if (typeof auto === 'number') compare(auto, pref); else setRes(null);
  }, [dims, pref]);                                          // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleInput = k => e => {
    const v = e.target.value.replace(/\D/g,'').slice(0,3);   // 3 桁制限
    setDims(d => ({ ...d, [k]: v }));
  };
  ...
  return (
    <div style={{ maxWidth:420, width:'100%', padding:'1vh 2vw',
      ...
      <div style={{background:'#f0f0f0', padding:'1vh', minHeight:'8vh',
        ...
        {matches.length ? (
          (()=>{const min=Math.min(...matches.map(m=>priceList[m])); return(
