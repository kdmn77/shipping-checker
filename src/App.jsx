...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const nums=[+l,+w,+h]; const [a,b,c]=[...nums].sort((x,y)=>y-x);
    const sum=nums.reduce((p,n)=>p+n,0); const f=[];
    if(a<=34&&b<=25&&c<=3) f.push('レターパックライト','レターパックプラス','クリックポスト');
    if(a<=32&&b<=23&&c<=3) f.push('ネコポス');
    if((a<=25&&b<=20&&c<=5)||(a<=34&&b<=25&&c<=5)) f.push('宅急便コンパクト');
    if(sum<=60&&a<=34&&c<=3) f.push('ゆうパケット');
    if(a<=24&&b<=17&&c<=7) f.push('ゆうパケットプラス');
    setMatches(f);
  },[dims,showCustom]);
  ...
  const handleKey=nextRef=>e=>{
    if(e.key==='Enter'&&nextRef?.current){ e.preventDefault(); nextRef.current.focus(); }
  };
    ...
    <>
      ...
      <style>{`
        @media (min-width:768px){
          ...
          .pref-btn    {width:auto !important; padding:12px 18px; font-size:15px;} /* ←縦幅UP */
              ...
              {list.map(p=>{
                ...
                return(
                  <button key={p} onClick={()=>handlePref(p)}
                    className="pref-btn"
                    style={{
                      width:'18%',padding:'1.2vh 0', /* ←縦幅UP (旧 .6vh) */
                      background:bg,
