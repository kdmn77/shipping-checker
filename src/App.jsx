...
const sizes = [
  ...
  200, 220, 240, 260, 'パケット系'      // ← “その他” を “パケット系” に変更
];
const priceList = {
  '宅急便コンパクト': 770,
  'ゆうパケット':      360,
...
export default function App() {
  ...
  const handleInput = key => e=>{
    ...
    if (key==='h') v = v.slice(0,1);  // 厚み:1桁
    else           v = v.slice(0,2);  // 縦横:2桁
    setDims(d => ({ ...d, [key]: v }));
  };
      ...
      {regionGroups.map(({name,list})=>(
        <div key={name} style={{marginBottom:'1vh'}}>
          <div style={{display:'flex',flexWrap:'wrap',gap:'1vw'}}>
            {list.map(p=>{
              const baseKey = name.split('・')[0];     // 地方キー
              let bg = regionColors[baseKey] || '#ccc';
              if (p==='東京') bg = regionColors['中国'];   // 赤
              if (p==='大阪') bg = regionColors['北海道']; // 青
