...
export default function App() {
  ...
  const onDimChange = (key, val) => {
    if (val.length>2) return;                   // 3 桁は無視（必要なら調整）
  ...
  return (
    <div style={{
      maxWidth:420, width:'100%', padding:'2vh 2vw',
      fontFamily:'-apple-system,BlinkMacSystemFont,"Helvetica Neue",sans-serif'
    }}>
      <h1 style={{fontSize:'6vw',margin:'0 0 2vh'}}>送料比較ツール</h1>

      {/* 結果エリア */}
      <div style={{background:'#f0f0f0',padding:'2vh 2vw',minHeight:'10vh',marginBottom:'2vh'}}>
        {showCustom ? matches.length ? (()=>{const min=Math.min(...matches.map(m=>priceList[m]));
          return <ul style={{margin:0,paddingLeft:'4vw'}}>
            {matches.map(m=>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:'1vw',marginBottom:'2vh'}}>
        {sizes.map(s=>(
          <button key={s} onClick={()=>handleSizeClick(s)} style={{
            width:'22%', padding:'1vh 0',
            background:s===size?'#0070f3':'#eee', color:s===size?'#fff':'#000',
            border:0,borderRadius:4,fontSize:'3.5vw'
          }}>{s}</button>
        ))}
      </div>
        ...
        <>
          ...
          <div style={{display:'flex',gap:'1vw',marginBottom:'2vh'}}>
            <input ref={lRef} type="number" placeholder="縦" value={dims.l}
              ...
              style={{width:'32%',padding:'1vh',fontSize:'4vw',border:'1px solid #ccc',borderRadius:4}} />
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:'1vw'}}>
        {allPrefectures.map(p=>{
          const bg = Object.entries(regionMap)
            .find(([,list])=>list.includes(p))?.[0];
          return (
            <button key={p} onClick={()=>handlePrefClick(p)} style={{
              width:'22%', padding:'1vh 0', marginBottom:'1vh',
              background:regionColors[bg]||'#ccc',
              color:p===prefecture?'#fff':'#000',
              border:p===prefecture?'2px solid #000':'0',
              borderRadius:4,fontSize:'3.2vw'
            }}>{p}</button>
          );
        })}
      </div>
